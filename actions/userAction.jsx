...
export const load_UserProfile = () => async (dispatch) => {
  try {
    ...
    if (userData !== "undefined" && userData && userData !== undefined ) {
     ...
     const config = { withCredentials: true }; // Add this line to send cookies
      const { data } = await axios.get("http://localhost:5001/api/v1/profile", config);
...
export function logout() {
  return async function (dispatch) {
    try {
      ...
      await axios.get(`http://localhost:5001/api/v1/logout`, { withCredentials: true }); // Add this line to send cookies // token will expired from cookies and no more user data access
...
export function updatePassword(userPassWord) {
  return async function (dispatch) {
    try {
      ...
      const { data } = await axios.put(
        `http://localhost:5001/api/v1/password/update`,
        userPassWord,
        config
      );
...
export const getAllUsers  = () =>async (dispatch) =>{
  ...
  try {
    ...
    const config = { withCredentials: true }; // Added this line to send cookies
    const { data } = await axios.get("http://localhost:5001/api/v1/admin/users", config);
...
export const getUserDetails = (id) => async (dispatch) => {
  try {
     ...
     const config = { withCredentials: true }; // Added this line to send cookies
     const { data } = await axios.get(`http://localhost:5001/api/v1/admin/user/${id}`, config);
...
export const updateUser = (id, userData) => async (dispatch) => {
  ...
  try {
     ...
     const config = {
      headers: { "Content-Type": "application/json" },
      withCredentials: true, // Added this line to send cookies
     };

     const { data } = await axios.put(
       `http://localhost:5001/api/v1/admin/user/${id}`,userData,
       config
...
export const deleteUser  =(id) => async (dispatch) =>{
  try {
       ...
       const config = { withCredentials: true }; // Added this line to send cookies
       const { data } = await axios.delete(`http://localhost:5001/api/v1/admin/user/${id}`, config);
