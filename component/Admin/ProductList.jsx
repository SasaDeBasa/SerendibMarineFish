...
import autoTable from "jspdf-autotable"; // Import the jsPDF-autotable library
...
function ProductList() {
  ...
  const exportToPdf = () => {
    ...
    doc.text("Product List Report", 14, 20); // (text, x-coordinate, y-coordinate)


    autoTable(doc, {
      startY: 30, 
      head: [['Product ID', 'Name', 'Stock', 'Price']],
      body: rows.map((row) => [row.id, row.name, row.stock, row.price]), // Map your table data
    });
